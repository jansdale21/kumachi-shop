<x-layouts.admin title="Kumachi Admin | Edit Supplier">
    <section class="admin-products-page">
        ...
        <section class="products-form-panel">
            <form class="products-form" method="POST" action="{{ route('admin.suppliers.update', $supplier) }}">
                @csrf
                @method('PUT')
                ...
                <div class="products-form-grid">
                    <div class="products-field">
                        ...
                        <input id="supplier_name" name="supplier_name" type="text" value="{{ old('supplier_name', $supplier->supplier_name) }}" required>
                ...
                <div class="products-field">
                    ...
                    <input id="address" name="address" type="text" value="{{ old('address', $supplier->address) }}">
                    <x-input-error :messages="$errors->get('address')" class="products-form-error" />
                </div>

                <div class="products-field">
                    <label>Supplies (Inventory Items)</label>
                        ...
                        @foreach ($inventories as $inventory)
                            <label class="products-checkbox-item" for="inventory_{{ $inventory->id }}">
                                <input
                                    ...
                                    name="inventory_ids[]"
                                    value="{{ $inventory->id }}"
                                    {{ in_array((string) $inventory->id, array_map('strval', old('inventory_ids', $selectedInventoryIds)), true) ? 'checked' : '' }}
                                >
                ...
                <div class="products-field">
                    ...
                    <textarea id="new_supplies" name="new_supplies" rows="4" placeholder="e.g. Brown Sugar Syrup&#10;Coconut Jelly">{{ old('new_supplies') }}</textarea>
                ...
                <div class="products-form-actions">
                    <button class="products-add-button" type="submit">Update Supplier</button>
                </div>
            </form>
        </section>
    </section>
