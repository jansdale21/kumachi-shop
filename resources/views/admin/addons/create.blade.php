<x-layouts.admin title="Kumachi Admin | Add Add-on">
    <section class="admin-products-page">
        ...
        <section class="products-form-panel">
            <form class="products-form" method="POST" action="{{ route('admin.addons.store') }}">
                @csrf
                ...
                <div class="products-form-grid">
                    <div class="products-field">
                        ...
                        <select id="inventory_id" name="inventory_id" required>
                            ...
                            @foreach ($inventories as $inventory)
                                <option value="{{ $inventory->id }}" {{ (string) old('inventory_id') === (string) $inventory->id ? 'selected' : '' }}>
                                    {{ $inventory->item_name }}
                    ...
                    <div class="products-field">
                        ...
                        <input id="inventory_usage_qty" name="inventory_usage_qty" type="number" step="0.01" min="0.01" value="{{ old('inventory_usage_qty', 1) }}" required>
                        ...
                        @foreach ($products as $product)
                            <label class="products-checkbox-item" for="product_{{ $product->id }}">
                                <input
                                    ...
                                    name="product_ids[]"
                                    value="{{ $product->id }}"
                                    {{ in_array((string) $product->id, array_map('strval', old('product_ids', [])), true) ? 'checked' : '' }}
                                >
                ...
                <div class="products-form-actions">
                    <button class="products-add-button" type="submit">Save Add-on</button>
                </div>
            </form>
        </section>
    </section>
