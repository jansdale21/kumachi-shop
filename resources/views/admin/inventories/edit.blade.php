<x-layouts.admin title="Kumachi Admin | Edit Inventory Item">
    <section class="admin-products-page">
        ...
        <section class="products-form-panel">
            <form class="products-form" method="POST" action="{{ route('admin.inventories.update', $inventory) }}">
                @csrf
                @method('PUT')
                <div class="products-form-grid">
                    <div class="products-field">
                        <label for="item_name">Item name</label>
                        <input id="item_name" name="item_name" type="text" value="{{ old('item_name', $inventory->item_name) }}" required>
                    ...
                    <div class="products-field">
                        ...
                        <select id="unit" name="unit" required>
                            @foreach ([
                                ...
                                <option value="{{ $unitValue }}" {{ old('unit', (string) $inventory->unit) === $unitValue ? 'selected' : '' }}>{{ $unitLabel }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="products-field">
                        ...
                        <input id="quantity" name="quantity" type="number" step="0.01" min="0" value="{{ old('quantity', $inventory->quantity) }}" required>
                        <x-input-error :messages="$errors->get('quantity')" class="products-form-error" />
                    </div>
                    <div class="products-field">
                        <label for="reorder_level">Reorder at</label>
                        <input id="reorder_level" name="reorder_level" type="number" step="0.01" min="0" value="{{ old('reorder_level', $inventory->reorder_level) }}" required>
                    ...
                    <div class="products-field">
                        ...
                        <select id="base_unit" name="base_unit" required>
                            @foreach ([
                                ...
                                <option value="{{ $unitValue }}" {{ old('base_unit', (string) ($inventory->base_unit ?? $inventory->unit)) === $unitValue ? 'selected' : '' }}>{{ $unitLabel }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="products-field">
                        ...
                        <input id="units_per_stock_unit" name="units_per_stock_unit" type="number" step="0.0001" min="0.0001" value="{{ old('units_per_stock_unit', (float) ($inventory->units_per_stock_unit ?? 1)) }}" required>
                        <x-input-error :messages="$errors->get('units_per_stock_unit')" class="products-form-error" />
                    </div>
                </div>

                <div class="products-form-actions">
                    <button class="products-add-button" type="submit">Update</button>
                </div>
            </form>
        </section>
    </section>
